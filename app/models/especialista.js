  ...
  var especialista = sequelize.define('especialista', {
    ...
    classMethods:{
			...
			encontrarEspecialidades: function(ciudadpasada, enfermedad, edad){ return sequelize
				  .query('select especialidad from especialistas where comunas_idcomunas= ? and e.id_especialista IN (select especialistas_id_especialista from enfermedades_tiene_especialistas where enfermedad_nombre_enfermedad= ?)',
				  { raw: true, replacements: [ciudadpasada, enfermedad], type: sequelize.QueryTypes.SELECT })
			}
